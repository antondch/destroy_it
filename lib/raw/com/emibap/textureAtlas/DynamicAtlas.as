...
public class DynamicAtlas
{
    ...
    static protected function getRealBounds(clip:DisplayObject):Rectangle
    {
        ...
        getRealBoundsInClipWithChilds(clip);
...
//        }
        ...
        //fix for union bounds in child objects
        //TODO: make it as recursive function
        function getRealBoundsInClipWithChilds(childClip:DisplayObject):void
        {
            var container:DisplayObjectContainer = childClip as DisplayObjectContainer;
            if (container)
            {
                for (var childNum:int = 0; childNum < container.numChildren; childNum++)
                {
                    getRealBoundsInClipWithChilds(container.getChildAt(childNum));
                }
            }
            if (childClip.filters.length > 0)
            {
                // filters
                var j:int = 0;
                //var clipFilters:Array = clipChild.filters.concat();
                var clipFilters:Array = childClip.filters;
                var clipFiltersLength:int = clipFilters.length;
                var tmpBData:BitmapData;
                var filterRect:Rectangle;
